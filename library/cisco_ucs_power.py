...
options:
    state:
        description:
         ...
         - if C(cycle-wait), will trigger a graceful OS shutdown and powercycle operation on the specified server.
         - if Ccycle-immediate), will trigger an immediate powercycle operation on the specified server.
        required: true
...
#    conn = ucshandle.UcsHandle(module)
    server = conn.login()
